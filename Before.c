	...
	do
	{
		...
		if (modifyReply == 'Y' || modifyReply == 'y')
		{
			...
			if (fOutput != NULL)
			{
				...
				{
					fwrite(&staff[i], sizeof(staff[i]), 1, fStaff);
					if (strcmp(modifyID, staff[i].staffID) == 0)
					{
						printf("Enter The Modify Staff ID                       : ");
						rewind(stdin);
						scanf("%s", &temporaryStaff[i].staffID);
						printf("Enter The Modify Staff Name                     : ");
						rewind(stdin);
						scanf("%[^\n]", &temporaryStaff[i].staffName);
						printf("Enter The Modify Staff Department               : ");
						rewind(stdin);
						scanf("%s", &temporaryStaff[i].dept);
						printf("Enter The Modify Staff Date Joined (DD-MM-YYYY) : ");
						rewind(stdin);
						scanf("%d %d %d", &temporaryStaff[i].datejoined.day, &temporaryStaff[i].datejoined.month, &temporaryStaff[i].datejoined.year);
						printf("Enter The Modify Staff Qualification            : ");
						rewind(stdin);
						scanf("%[^\n]", &temporaryStaff[i].qualification);
	...
	do 
	{
		...
		do 
		{
			...
			if (answer == 1)
			{
				...
				rewind(stdin);
				if (strcmp(idSearchDelete, staff.staffID) == 0)
				{
					printf("The Deleted Will Succesfull \n");
					fwrite(&staff, sizeof(Staff), 1, fStaff);
				}
